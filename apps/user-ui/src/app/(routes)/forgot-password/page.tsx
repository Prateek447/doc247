...
const ForgotPassword = () => {
  ...
  const verifyOtpMutation  =  useMutation({
    ...
    onError: (error: AxiosError) => {
        const errorMessage = (error.response?.data as {message? : string })?.message || "Can't verify otp. Try again!";
        setServerError(errorMessage);
    }
  })
  ...
  const resetPasswordMutation = useMutation({
    ...
    onError: (error: AxiosError) => {
        const errorMessage = (error.response?.data as {message? : string })?.message || "Can't reset password ! Try again";
        setServerError(errorMessage);
    }
  })
        ...
        <div className="md:w-[480px] p-8 bg-white shadow rounded-lg">
         { step === "email" && (
          ...
          <form onSubmit={handleSubmit(onSubmitEmail)}>
            ...
            <button disabled={requestOtpMutation.isPending} type="submit" className="w-full text-lg cursor-pointer bg-black text-white py-2 rounded-lg mt-3">
            ...
            <>
              <div>
                               <h3 className="text-xl font-semibold text-center mb-4">Enter OTP</h3>
                                ...
                                <button disabled={verifyOtpMutation.isPending} className="w-full mt-4 text-lg cursor-pointer bg-blue-500 text-white py-2 rounded-lg">
                                 {verifyOtpMutation.isPending ? "Verifying..." : "Verify OTP"}
                                 </button>
                                 <p className="text-center text-sm mt-4">
                                     {canResend ? ( <button onClick={() => requestOtpMutation.mutate({email: userEmail!})} className="text-blue-500 cursor-pointer">Resend OTP</button> ) : (`Resend OTP in ${timer}`)}
                                 </p>
            ...
            <h3 className="text-xl font-semibold text-center mb-4">Reset Password</h3>
              ...
              {...register("password", {
               ...
               />
                ...
                <button disabled={resetPasswordMutation.isPending} type="submit" className="w-full text-lg cursor-pointer bg-black text-white py-2 rounded-lg mt-3">
